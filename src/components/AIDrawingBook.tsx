...
const AIDrawingBook: React.FC<AIDrawingBookProps> = ({ onBack }) => {
  ...
  const recognizeImageWithGemini = async (imageData: string): Promise<string> => {
    try {
      if (!GeminiService.getApiKey()) {
        throw new Error('Gemini API key not configured');
      }

      const result = await GeminiService.recognizeText(imageData);
      
      // If no text was found, try to get a general description
      if (!result.text || result.text.toLowerCase().includes('no text')) {
        // Use a different prompt for general image description
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GeminiService.getModel()}:generateContent?key=${GeminiService.getApiKey()}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: "Describe what you see in this image in simple terms. Focus on the main objects, shapes, animals, or things that could be turned into a coloring book page. Be descriptive but concise." },
        ...
        const descriptionResult = await response.json();
        return descriptionResult.candidates[0].content.parts[0].text.trim();
      }
  ...
  const getDrawingIdea = async () => {
    ...
    try {
      if (!GeminiService.getApiKey()) {
        // Fallback to predefined ideas if Gemini is not available
        const ideas = [
          "A friendly robot drinking a milkshake",
          "A cat wearing a superhero cape",
          "A rainbow bridge over a magical forest",
          "A smiling sun with sunglasses",
          "A house made of candy and cookies",
          "A dragon playing with butterflies",
          "A tree with books growing as fruits",
          "A spaceship shaped like a banana",
          "A unicorn dancing in the clouds",
          "A fish swimming in the stars"
        ];
        
        const randomIdea = ideas[Math.floor(Math.random() * ideas.length)];
        setCurrentPrompt(randomIdea);
        return;
      }
  ...
  const enhanceDrawing = async () => {
    ...
    try {
      ...
      recognizedDescription = await recognizeImageWithGemini(imageData);
  ...
  const generateStory = async () => {
    ...
    try {
      if (!GeminiService.getApiKey()) {
        // Fallback story if Gemini is not available
        const fallbackStories = [
          "Once upon a time, there was a magical drawing that came to life and had wonderful adventures!",
          "In a land of colors and creativity, your drawing became the hero of an amazing story!",
          "Your beautiful creation sparked joy and wonder in everyone who saw it!"
        ];
