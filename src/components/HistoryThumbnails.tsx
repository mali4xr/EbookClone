        ...
        <div className="flex overflow-x-auto gap-2 pb-1">
          {history.map((item, idx) => (
            <div key={idx} className="flex flex-col items-center flex-shrink-0">
              <div
                className={`relative rounded-2xl border-4 cursor-pointer transition-all duration-300 transform hover:scale-10 ${
              ...
              <div className="mt-2 w-8 h-8 flex items-center justify-center rounded-full bg-gradient-to-br from-purple-500 to-pink-500 text-white font-bold text-sm shadow-lg">
                {idx + 1}
              </div>
            </div>
          ))}
